...
- troubleshooting: see if there's a way to pre-load the pseudoclass 'Connect With Me' images so that there isn't the unsightly flash when the hover state images first load on contact. (STILL WORKING ON THIS! It seems to work ok in some browsers but not so much in FF - still testing, though.)
